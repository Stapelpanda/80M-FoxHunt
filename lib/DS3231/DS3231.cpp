...
volatile bool DS3231::INT0_Interrupt = 0;
...
{
    uint8_t clearTime[6] = {0,0,0,0,0,0};
    i2c_init();
    i2c_status_t res = I2C_ok;
    // Reset all stuff
    res = (i2c_status_t)(disableAlarm(DS3231_ALARM_1) |
    ...
    res = (i2c_status_t)(res | 
            i2c_writeReg(DS3231_ADDRESS, 0, clearTime, 6));
    return res;
}
...
{
    uint32_t currentTime = current->toSeconds();
    uint32_t nextTime = next->toSeconds();
...
uint32_t DS3231_Time::toSeconds()
{
